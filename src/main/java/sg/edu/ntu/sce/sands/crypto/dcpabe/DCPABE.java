...
import sg.edu.ntu.sce.sands.crypto.dcpabe.generators.BLS01ParametersGenerator;
...
public class DCPABE {
    ...
    public static void setBLSParameters()
    {
    ...
    public static BLS01Parameters getBLSParameters()
    {
        return new BLS01Parameters(parameters);
    }

    public static AuthorityKeys authoritySetup(String authorityID, GlobalParameters GP, Attributes[] attr) {
        ...
        SecretKey sk = new SecretKey(ai.toBytes(), yi.toBytes());
        authorityKeys.secretAA = sk;

        PublicKey pk = new PublicKey(eg1g1.powZn(ai).toBytes(), GP.getG1().powZn(yi).toBytes());
        authorityKeys.publicAA = pk;

        for(Attributes a: attr)
        {
            AttributeValues publicKey = new AttributeValues();
            AttributeValues secretKey = new AttributeValues();
            for(String val: a.attributeValues)
            {
                Element ti = pairing.getZr().newRandomElement().getImmutable();
                Element giti = GP.getG1().powZn(ti);

                publicKey.addAttributeValues(val, giti.toBytes());
                secretKey.addAttributeValues(val, ti.toBytes());
            }
            authorityKeys.mapOf_AA_PK.put(a.attributeName, publicKey);
            authorityKeys.mapOf_AA_SK.put(a.attributeName, secretKey);
        }
